...
# Prefer per-user cargo/rustup locations to avoid root installs
...
# If the script is run under sudo, prefer the invoking user's home so we
# don't default to root (/root or /opt/root) or a container root home.
...
# Accept an existing env var, but normalize to a single directory if a
# colon-separated list was accidentally provided (CARGO_HOME should be a
# single path). Use the first component as the canonical location.
_CARGO_RAW="${CARGO_HOME:-${INVOKER_HOME}/.cargo}"
_RUSTUP_RAW="${RUSTUP_HOME:-${INVOKER_HOME}/.rustup}"
# If someone accidentally set CARGO_HOME to a colon-separated list, take first
...
# Ensure cargo bin is at the front of PATH so newly-installed tools are found
export PATH="$CARGO_HOME/bin:$PATH"
echo "Using PATH: $PATH"

# Prefer the rustup binary installed in $CARGO_HOME/bin so we do not
# accidentally invoke a system-wide rustup that uses a system RUSTUP_HOME
# (for example /opt/rust). If no rustup is available, fail early.
...
# If rustup exists but no default toolchain is configured, set the stable
# toolchain as default so `cargo install` and other commands work.
...
# Fail early if cargo/rustup directories are owned by root. This prevents
# `cargo install` from trying to write into /root/.cargo when scripts were
# previously run with sudo.
...
# If wasm-pack was installed via cargo, it may place helper binaries under
# $HOME/.cache/.wasm-pack/.../bin. Add that to PATH if present so installed
# tools like wasm-bindgen are runnable.
...
# Install wasm-bindgen CLI only if missing (or if FORCE_CARGO_INSTALL=1)
...
# Step 5: Patch WASM for VS Code webview compatibility
echo ""
echo "=== Step 5: Patching WASM for VS Code webview compatibility ==="
# VS Code Remote webviews restrict WebAssembly.Table.grow() even though the
# browser supports reference-types. We patch the wasm binary to:
# 1. Pre-allocate 4 extra externref table slots (128 -> 132)
# 2. Fix the __wbindgen_externrefs export to point to the externref table
# We also patch the JS to use a try/catch fallback for table.grow().

# Check for wabt tools
if ! command -v wasm2wat >/dev/null 2>&1 || ! command -v wat2wasm >/dev/null 2>&1; then
    ...
    # Patch the JS file to handle table.grow() failure gracefully
    if [ -f "$JS_FILE" ]; then
...
# Pattern to match the original __wbindgen_init_externref_table function
...
# Step 6: Compile TypeScript extension
echo ""
...
# Ensure the target 'out' is a directory (avoid collision if a file named 'out' was copied)
...
if [ -d "$HOME/.vscode-server" ]; then
    ...
    # Prefer rsync for safety if available, fallback to cp -a
    if command -v rsync >/dev/null 2>&1; then
        rsync -a "$EXTENSION_DIR/" "$REMOTE_SERVER_EXT_DIR/"
    else
        cp -a "$EXTENSION_DIR/." "$REMOTE_SERVER_EXT_DIR/"
    fi
    # Ensure ownership is correct for current user; ignore errors where not allowed
...
if [ -x "$VSIX_SCRIPT" ]; then
    ...
    # Call the packager using 'bash' to avoid tcsh/interactive differences.
